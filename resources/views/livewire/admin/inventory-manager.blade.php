<div>
    <style>
        ...
        .inv-title { font-size: 1.75rem; font-weight: 900; background: linear-gradient(135deg, #f97316, #dc2626); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; }
        .btn-new-inv {
        ...
        .inv-alert-count {
            background: #dc2626; color: #fff; font-weight: 800; font-size: 0.75rem;
            padding: 0.2rem 0.6rem; border-radius: 50px; margin-left: 0.25rem;
        }

        /* Table */
        .inv-table-wrap {
            background: #111; border: 1px solid #222; border-radius: 16px; overflow: hidden;
        }
        .inv-table { width: 100%; border-collapse: collapse; }
        .inv-table thead { background: #0a0a0a; }
        .inv-table th {
            padding: 0.85rem 1rem; text-align: left; font-size: 0.7rem; font-weight: 700;
            color: #555; text-transform: uppercase; letter-spacing: 0.08em; border-bottom: 1px solid #1e1e1e;
        }
        .inv-table td {
            padding: 0.85rem 1rem; font-size: 0.85rem; color: #ccc; border-bottom: 1px solid #1a1a1a;
        ...
        .inv-product-name { font-weight: 700; color: #e5e5e5; }
        .inv-variant-name { font-size: 0.75rem; color: #666; margin-top: 0.15rem; }
        .inv-branch-tag {
            display: inline-block; background: #1a1a1a; border: 1px solid #2a2a2a;
            padding: 0.2rem 0.6rem; border-radius: 50px; font-size: 0.7rem; font-weight: 600; color: #888;
        }
        ...
        .btn-action {
            background: #111; border: 1px solid #2a2a2a; padding: 0.35rem 0.6rem;
        ...
        .inv-modal {
            background: #111; border: 1px solid #222; width: 100%; max-width: 480px;
        ...
        .inv-modal-header h3 { font-size: 1.15rem; font-weight: 800; color: #fff; }
        .inv-modal-body { padding: 1.25rem 1.5rem; }
        .inv-modal-footer { padding: 1.25rem 1.5rem; border-top: 1px solid #222; background: #0a0a0a; display: flex; gap: 0.75rem; justify-content: flex-end; }
        .inv-modal-close {
            background: transparent; border: 1px solid #222; color: #666; width: 36px; height: 36px;
        ...
        .inv-form-label { display: block; font-size: 0.75rem; font-weight: 700; color: #888; text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 0.4rem; }
        ...
        .btn-modal-delete {
            padding: 0.7rem 1.5rem; background: #dc2626; color: #fff;
            border: none; border-radius: 12px; font-weight: 800; font-size: 0.85rem; cursor: pointer; transition: all 0.2s;
        }
        .btn-modal-delete:hover { background: #ef4444; }

        .inv-empty { text-align: center; padding: 3rem; color: #444; }
        ...
        .inv-delete-warn p { color: #ccc; font-size: 0.9rem; margin-bottom: 0.5rem; }
    ...
    <div class="inv-header">
        <h1 class="inv-title">📦 Inventario por Sucursal</h1>
                ...
                <tbody>
                    @foreach($inventoryList as $inv)
                        <tr>
                            ...
                            <td style="text-align:center; color: #666;">{{ $inv->minimum_alert }}</td>
                            <td style="text-align:center;">
                                @if($inv->minimum_alert > 0 && $inv->stock_quantity <= 0)
                                    <span class="alert-badge alert-badge-danger">⛔ Agotado</span>
                                @elseif($inv->minimum_alert > 0 && $inv->stock_quantity <= $inv->minimum_alert)
                                    <span class="alert-badge alert-badge-warn">⚠️ Bajo</span>
                                @else
    ...
    <div class="inv-modal-overlay">
        <div class="inv-modal">
            <div class="inv-modal-header">
                <h3>{{ $isEdit ? '✏️ Editar Registro' : '📦 Nuevo Registro de Inventario' }}</h3>
            ...
            <div class="inv-modal-body">
                ...
                <div class="inv-form-group">
                    ...
                    <input wire:model="formMinimumAlert" type="number" min="0" class="inv-form-input" placeholder="0">
                    @error('formMinimumAlert') <p class="inv-error">{{ $message }}</p> @enderror
                    <p style="color: #555; font-size: 0.75rem; margin-top: 0.35rem;">
        ...
        <div class="inv-modal">
            ...
            <div class="inv-modal-body">
                <div class="inv-delete-warn">
                    ...
                    <p style="font-size: 0.75rem; color: #888; margin-top: 0.75rem;">
            ...
            <div class="inv-modal-footer">
                ...
                <button wire:click="deleteInventory" class="btn-modal-delete">🗑️ Sí, Eliminar</button>
            </div>
        </div>
    </div>
    @endif
